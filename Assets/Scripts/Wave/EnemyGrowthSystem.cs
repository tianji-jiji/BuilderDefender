...
public class EnemyGrowthSystem
{
    ...
    public EnemyRuntimeStats BuildStats(EnemySo enemySo, int waveIndex, WaveRuleSystem.EnemyKind enemyKind, DefensePowerSnapshot playerPowerSnapshot)
    {
        ...
        if (enemyKind == WaveRuleSystem.EnemyKind.Boss)
        {
    ...
    {
        float responseStrength = _config ? _config.PlayerPowerResponseStrength : DEFAULT_PLAYER_POWER_RESPONSE;
        if (enemyKind == WaveRuleSystem.EnemyKind.Boss)
        {
    ...
    {
        return _config ? _config.HealthMultiplierByWave : null;
    }

    // 获取护甲成长曲线。
    private AnimationCurve GetArmorBonusCurve()
    {
        return _config ? _config.ArmorBonusByWave : null;
    }

    // 获取攻击成长曲线。
    private AnimationCurve GetAttackMultiplierCurve()
    {
        return _config ? _config.AttackMultiplierByWave : null;
    }

    // 获取移速成长曲线。
    private AnimationCurve GetMoveSpeedMultiplierCurve()
    {
        return _config ? _config.MoveSpeedMultiplierByWave : null;
    }
