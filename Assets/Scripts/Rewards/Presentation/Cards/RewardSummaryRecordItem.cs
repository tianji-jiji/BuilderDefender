...
public class RewardSummaryRecordItem : MonoBehaviour
{
    ...
    // 刷新单条奖励记录的 UI 文本。
    public void SetRecord(RewardCardRecord rewardRecord)
    {
        ...
        RefreshRarityBorder(rewardRecord.RewardCard.Rarity);
    }

    // 刷新有效奖励记录的所有文本。
    private void RefreshText(RewardCardRecord rewardRecord)
    {
        RewardCardSo rewardCard = rewardRecord.RewardCard;
        if (cardNameText)
        {
            cardNameText.text = rewardCard.CardName;
        }
        ...
        if (rarityText)
        {
            rarityText.text = rewardCard.Rarity.ToString();
        }
