...
public class RewardGainToast : MonoBehaviour
{
    ...
    private void HandleRewardApplied(RewardAppliedContext context)
    {
        ...
        RefreshText(context.AppliedRecord);
        RefreshRarityColor(context.AppliedRecord.RewardCard.Rarity);
        PlayFeedback();
        RestartAutoHide();
    }
    ...
    // 刷新本次获得奖励的文本。
    private void RefreshText(RewardCardRecord appliedRecord)
    {
        RewardCardSo rewardCard = appliedRecord.RewardCard;
        if (cardNameText)
        {
            cardNameText.richText = true;
            cardNameText.text = $"{CARD_NAME_PREFIX}{rewardCard.CardName}";
        }
