...
{
    private static readonly List<DefenseTowerCombatSystem> DefenseTowerSystemList = new();
    ...
    // 注册防御塔战斗系统。
    public static void RegisterDefenseTowerSystem(DefenseTowerCombatSystem defenseTowerCombatSystem)
    {
        AddUnique(DefenseTowerSystemList, defenseTowerCombatSystem);
    }

    // 取消注册防御塔战斗系统。
    public static void UnregisterDefenseTowerSystem(DefenseTowerCombatSystem defenseTowerCombatSystem)
    {
        DefenseTowerSystemList.Remove(defenseTowerCombatSystem);
    }
    ...
    {
        int count = 0;
        foreach (DefenseTowerCombatSystem defenseTowerSystem in DefenseTowerSystemList)
        {
    ...
    // 判断指定防御塔附近是否存在另一座防御塔。
    public static bool HasNearbyDefenseTower(DefenseTowerCombatSystem sourceDefenseTowerCombatSystem, float radius)
    {
        if (!sourceDefenseTowerCombatSystem || radius <= 0f)
        {
            return false;
        }

        float radiusSqr = radius * radius;
        Vector3 sourcePosition = sourceDefenseTowerCombatSystem.transform.position;

        foreach (DefenseTowerCombatSystem defenseTowerSystem in DefenseTowerSystemList)
        {
            if (!defenseTowerSystem || defenseTowerSystem == sourceDefenseTowerCombatSystem)
            {
                continue;
            }
    ...
    // 获取指定防御塔对应的升星按钮。
    public static BuildingUpgradeButton GetUpgradeButton(DefenseTowerCombatSystem defenseTowerCombatSystem)
    {
        ...
        foreach (BuildingUpgradeButton upgradeButton in UpgradeButtonList)
        {
            if (upgradeButton && upgradeButton.DefenseTowerCombatSystem == defenseTowerCombatSystem)
            {
