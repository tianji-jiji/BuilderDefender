...
/// </summary>
public static class DefenseTowerPowerEvaluator
{
    private const float DEFAULT_POWER_MULTIPLIER = 1f;

    // 根据当前防御塔奖励状态创建战力快照。
    public static DefenseTowerPowerSnapshot CreateSnapshot(DefenseTowerActiveRewards activeRewards)
    {
        if (activeRewards == null)
        {
            return DefenseTowerPowerSnapshot.Default();
        }
    ...
    // 将攻击间隔倍率转换为战力倍率。
    private static float GetAttackSpeedPowerMultiplier(DefenseTowerActiveRewards activeRewards)
    {
