...
public class TowerStatCalculator
{
    ...
    private readonly TowerCombatSystem sourceTowerCombatSystem;
    ...
    private TowerActiveRewards ActiveRewards => RewardRuntimeCoordinator.Instance
        ? RewardRuntimeCoordinator.Instance.TowerRewards.ActiveRewards
        : null;

    public TowerStatCalculator(TowerCombatSystem sourceTowerCombatSystem, int baseAttackDamage, float baseArrowGenerateRate, float baseDetectRadius)
    {
    ...
    {
        TowerActiveRewards activeRewards = ActiveRewards;
    ...
    private float GetThreeStarResonanceMultiplier()
    {
        ...
        int threeStarCount = TowerRegistry.GetThreeStarDefenseTowerCount();
    ...
    private float GetFinalDefenseTowerAttackDamageMultiplier()
    {
        return RewardRuntimeCoordinator.Instance
            ? RewardRuntimeCoordinator.Instance.TowerRewards.GetFinalDefenseTowerAttackDamageMultiplier()
            : 1f;
    }
    ...
    {
        TowerActiveRewards activeRewards = ActiveRewards;
        return sourceTowerCombatSystem
               && activeRewards is { LinkRadius: > 0f }
               && TowerRegistry.HasNearbyDefenseTower(sourceTowerCombatSystem, activeRewards.LinkRadius);
    }
